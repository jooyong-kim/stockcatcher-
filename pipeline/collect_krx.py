...
# VOL15: 차트 다년(1/2/3년) 조회 지원 — 수집·저장 범위를 약 3년으로 확대.
# 지표 계산(analyze_ticker)은 전체 배열의 최신값을 쓰므로 값 자체는 종전과 동일하고,
# charts 파일에 더 긴 히스토리를 담아 프론트에서 1/2/3년 버튼으로 펼쳐볼 수 있게 함.
LOOKBACK_DAYS = 760        # 지표 계산 + 차트용 거래일 수 (약 3년, 거래일 ≈ 250/년)
SPARK_LEN = 60             # 프론트 미니차트 길이
CHART_LEN = 760            # 종목별 풀차트 데이터 길이 (거래일, 약 3년 — 프론트에서 1/2/3년으로 잘라 표시)
...
def analyze_ticker(code, name, market, op, h, lo, c, v, amt):
    ...
    # ── 박스권(횡보) 판정용 ──
    box_win = 60  # 박스 판정 기간
    bw_hi = _hh(h, box_win, i)
    bw_lo = _ll(lo, box_win, i)
    box_range_pct = (bw_hi - bw_lo) / (bw_lo or 1e-9) * 100  # 박스 폭(고저 범위 %)
    box_pos = (c[i] - bw_lo) / ((bw_hi - bw_lo) or 1e-9) * 100  # 박스 내 현재 위치(0=바닥,100=천장)
    # 추세 기울기: 60일 종가 선형회귀 기울기를 일평균 %로
    if i >= box_win - 1:
        ys = c[i - box_win + 1: i + 1]
        xs = np.arange(box_win)
        slope = np.polyfit(xs, ys, 1)[0]
        box_slope_pct = slope / (np.mean(ys) or 1e-9) * 100  # 하루당 추세 기울기 %
    else:
        box_slope_pct = 0.0
    # VOL14 수정: boxFlatPct(박스 수평성)가 이전까지 계산·출력되지 않아
    # 프론트(VOL12~13)의 박스권 필터가 실데이터에서 항상 0종목으로 나오던 버그의 원인이었음.
    # 프론트 JS의 flatness() 로직과 동일하게 상단·하단 각각의 평평함을 구해 더 큰 값을 사용.
    box_start = max(0, i - box_win + 1)
    h_seg = h[box_start: i + 1]
    l_seg = lo[box_start: i + 1]

    def _flatness(arr, top):
        if len(arr) < 10:
            return 100.0
        srt = np.sort(arr)
        k = max(3, len(arr) // 5)
        band = srt[-k:] if top else srt[:k]
        m = float(np.mean(band))
        sd = float(np.std(band))  # population std — JS 쪽과 동일한 정의
        return sd / (abs(m) or 1e-9) * 100

    box_flat_pct = max(_flatness(h_seg, True), _flatness(l_seg, False))
    ...
    return {
        ...
        "boxRangePct": f(box_range_pct), "boxPos": f(box_pos), "boxSlopePct": f(box_slope_pct, 3),
        "boxFlatPct": f(box_flat_pct), "boxHi": f(bw_hi, 0), "boxLo": f(bw_lo, 0),
...
def main():
    ...
    # VOL15: 다년 수집은 거래일 수에 비례해 시간이 늘어남 (KRX 호출 2회/일 × SLEEP)
    est_min = len(dates) * 2 * SLEEP / 60
    print(f"  ⏱️ 예상 수집 시간 약 {est_min:.0f}분 (거래일 {len(dates)}일 기준). "
          f"다년 데이터라 첫 실행은 다소 길 수 있어요.", flush=True)
